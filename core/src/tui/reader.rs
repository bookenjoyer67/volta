...
impl ReaderState {
    ...
    pub fn render(
        ...
        // Text area
        let text_area = Rect::new(area.x, area.y + 1, area.width, area.height.saturating_sub(3));
        ...
        for i in start..end {
            ...
            if i == self.cursor_line() {
                let cursor_offset_in_line =
                    self.cursor_word.saturating_sub(self.line_word_offsets[i]);
                ...
                for (wi, word) in words.iter().enumerate() {
                    ...
                    if wi == cursor_offset_in_line {
                        // Cursor word — highlight in hot pink
        ...
        let pages = (self.wrapped_lines.len().saturating_add(visible.saturating_sub(1)))
            / visible.max(1);
